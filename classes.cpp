...
void Handler::coll_speed_change_dyn(std::list<Object_dynamic>::iterator &dyn1, std::list<Object_dynamic>::iterator &dyn2) {
    ...
    glm::vec3 tangent_speed_1 = dyn1->get_speed() * glm::sin(alpha1);
    glm::vec3 normal_speed_1 = dyn1->get_speed() * glm::cos(alpha1) * glm::sin(beta1);
    glm::vec3 tangent_speed_sin_1 = dyn1->get_speed() * glm::cos(alpha1) * glm::cos(beta1);

    glm::vec3 tangent_speed_2 = dyn1->get_speed() * glm::sin(alpha2);
    glm::vec3 normal_speed_2 = dyn1->get_speed() * glm::cos(alpha2) * glm::sin(beta2);
    glm::vec3 tangent_speed_sin_2 = dyn1->get_speed() * glm::cos(alpha2) * glm::cos(beta2);
    ...
    glm::vec3 new_speed_1 = tangent_speed_1 + tangent_speed_sin_1 - normal_speed_1 + normal_speed_2;
    glm::vec3 new_speed_2 = tangent_speed_2 + tangent_speed_sin_2 - normal_speed_2 + normal_speed_1;
...
void Handler::player_update() {
    ...
    for(ac; ac!= act_elems.end(); ac++) {
        coll_type = collision_act(player, ac);
        if(coll_type == X_COLLISION) {
            player.get_center().x = old_center.x;
        }
        if(coll_type == Y_COLLISION) {
    ...
    for(acr; acr != actr_elems.end(); acr++) {
        coll_type = collision_actr(player, acr);
        if(coll_type == X_COLLISION) {
            player.get_center().x = old_center.x;
        }
        if(coll_type == Y_COLLISION) {
